...
### for whole brown corpus ###
bigram3=Counter(ngrams(new_brown,2))
del(bigram3['<s>','<e>'])
trigram3=Counter(ngrams(new_brown,3))
...
### dev set counts###    
unigram2 = {}
for token in new_dev1:
  if token not in unigram2:
    unigram2[token] = 1
  else:
    unigram2[token] += 1
...
def generate_sentence(word1,N,p):
    ...
    if k<=20:
        one=" ".join(sentence)
        two=" ".join(sentence3)
        print("\nsentence generated from brown corpus:")
        print(one,two)
    else:
